...
## Логирование результатов функций с использованием декоратора
Этот проект предоставляет простой декоратор log для логирования результатов вызовов функций. 
Декоратор может записывать логи в консоль или в файл.
## Установка
...
## Использование
Импортируйте декоратор log и применяйте его к функциям, которые вы хотите логировать:
```python
...
print(result)
```
##### Этот пример применяет декоратор log к функции foo, которая складывает два числа. Результат будет записан в файл mylog.txt и выведен в консоль.

Параметры
- filename (Optional[str]): Путь к файлу, куда будут записываться логи. Если None, логи будут выводиться только в консоль.
...
```
### Тестирование
Запустите тесты, чтобы убедиться, что декоратор работает корректно:
```python
pytest tests/
```